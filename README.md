...
· `--topics <chat-id>` · `--channels <a,b,c>` · `--wake-agent <name>` · `--status-icons` · `--no-markdown` (alias `--plain`). The token may also come from `$TELEGRAM_BOT_TOKEN`, the Groq key from
`$GROQ_API_KEY`.
...
| **Status icons** | **Off by default.** `--status-icons` drives each agent topic's icon from its status (✅ idle · ⚡️ working · 👀 waiting · ☕️ offline) — the code is intact but inert, because the constant churn read as more noise than signal. When it is off, any icon a previous run stamped is cleared once at startup rather than left frozen mid-status. Note that presence alone under-reports `working` (it publishes only on a human prompt, so a mesh-triggered turn reads `idle` throughout), so with the flag on, `paw status --json` is polled for the truer signal when `paw` is present.
