...
| **Status at a glance** | The topic **icon** tracks the agent: ✅ idle · ⚡️ working · 👀 waiting · ☕️ offline. Status is NOT taken from cotal presence alone — presence publishes `working` only on a human prompt, so an agent working a **mesh-triggered** turn reads `idle` for the whole turn and emits no event at all ([filed upstream](https://cotal.ai)). When `paw` is on the machine, `paw status --json` supplies the truer signal (it scans each transcript's turn markers) and a poller restamps the icon; the agent's own `working`/`waiting` claim still wins, and with no `paw` present the tracker simply has **no opinion** and behaves exactly as before rather than reporting everyone idle. | `icon_color` is immutable once a topic exists, so a custom emoji is the only icon that can change — and a bot may only use Telegram's default topic-icon pack (which has no traffic lights), so these are the closest it allows. Re-stamped only when the status actually changes, and persisted, so a heartbeat or a bridge restart doesn't bury each topic under `changed the icon` notices. |
